...
export default function JobDetails() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.errors) {
                ...
                setFormData({ full_name: '', email: '', resume_url: '', cover_letter: '' });
            }
        } catch (err) {
    ...
    if (loading) return <div className="p-20 text-center text-navy font-bold">Loading...</div>;
    if (!job) return <div className="p-20 text-center text-navy font-bold">Job not found</div>;
            ...
            <main className="px-16 py-16 flex gap-12 max-w-7xl mx-auto">
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-6 mb-10">
                        ...
                        <div>
                            ...
                            <p className="text-body-text text-lg">{job.company} • {job.location} • {job.type}</p>
                        </div>
                    </div>
                    ...
                    <div className="prose prose-slate max-w-none">
                        ...
                        <p className="text-body-text leading-relaxed mb-8 whitespace-pre-line">{job.responsibilities || 'Not specified'}</p>

                        <h2 className="text-navy text-2xl font-bold mb-4">Requirements</h2>
                        <p className="text-body-text leading-relaxed mb-8 whitespace-pre-line">{job.requirements || 'Not specified'}</p>
                    </div>
                </div>
                ...
                <div className="w-[450px]">
                    <div className="p-8 border border-gray-100 bg-zinc-50 rounded-sm sticky top-10">
                        ...
                        {message && (
                            <div className={`p-4 mb-6 rounded text-sm font-bold ${message.includes('Error') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                                {message}
                            </div>
                        )}
